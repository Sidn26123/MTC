...
const ReadingPage = () => {
    ...
    const actions = useChapterActions(); // ✅ Gọi hook đúng chỗ
    let novelSlug = useCurrentNovelSlug();
    let chapterIdx = useCurrentChapterIdx();

    // const increaseChapterIdx = useIncreaseChapterIdx();
    // const decreaseChapterIdx = useDecreaseChapterIdx();


    useEffect(() => {
        console.log("Chapter Index:", chapterIdx);
        getCurrentChapterContent({novelSlug: novelSlug, chapterIdx: chapterIdx}).then((r) => {
            ...
            <div>
                ...
                <div className={"mx-2 mt-4"}>
                    <h2 className={"text-lg text-center text-balance"}>
                        <a className="text-title font-semibold" href="https://metruyencv.com/truyen/dao-gioi-thien-ha">Đạo
                            Giới Thiên Hạ</a>
                    </h2>
                    <h3 className="text-xs text-center text-gray-500"> Dạ Hành Nguyệt </h3>
                    <div className="flex justify-center space-x-2 items-center px-2 mt-4">
                        ...
                        <div><h2 className="text-center text-gray-600 dark:text-gray-400 text-balance"> Chương 1760:
                            Huyết Võng biến mất </h2></div>
...
const Content = ({ content }) => {
    ...
    console.log("Content:", content, lines);
    return (
        <>
