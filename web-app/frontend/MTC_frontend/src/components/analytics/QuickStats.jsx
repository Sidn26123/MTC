...
export function QuickStats() {
    ...
    return (
        <div className="bg-gray-600 rounded-lg p-6 mb-8">
            ...
            <RatingBars novelId={currentNovel.id}/>
...
import { getRatingOfNovel, getRatingOfNovelGroupByStart } from '../../services/feedbackService.js';
...
export function RatingBars({ novelId }) {
    // const maxCount = Math.max(...Object.values(ratingData), 1); // đảm bảo > 0 tránh chia 0
    const [rating, setRating] = useState({});


// Tìm giá trị lớn nhất để tính phần trăm
    const maxCount = Math.max(...Object.values(ratingData), 1); // tránh chia cho 0

    useEffect(() => {
        getRatingOfNovelGroupByStart(novelId).then((response) => {
